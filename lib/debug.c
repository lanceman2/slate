...
// This file is compiled into libslate.so
...
// libslate.so will not be able to get more spew from code in
// libslate.so, but other code may have had higher spew levels if
...
// This is where the user of libslate.so can quiet down the code in the
...
void sl_spew(int levelIn, FILE *stream, int errn,
        ...
        int lineNum, const char *func)
{
    ...
    if(slAssertAction)
        ...
        slAssertAction(stream, file, lineNum, func);
