...
which use functions in libslate.so.  If your not following this,
...
slSetSpewLevel() and/or the compile time default spew level that was
the compiled in default.

If at the time libslate was compiled the defining of the macros \c
...
Every API has crap like this, get over it.  We keep our slate crap
small, and independent.  We know this crap is not end user friendly, but
without this shit, writing this slate code is impossible, bugs would
...
installed as include/slate_debug.h.  There are a very small amount
...
SL_EXPORT
void (*slAssertAction)(FILE *stream, const char *file,
        int lineNum, const char *func);

SL_EXPORT
void sl_spew(int level, FILE *stream, int errn, const char *pre,
...
SL_EXPORT
void sl_assertAction(FILE *stream, const char *file,
        int lineNum, const char *func);


SL_EXPORT
int slGetLibSpewLevel(void);


SL_EXPORT
void slSetSpewLevel(int level);
...
#  define _SPEW(level, stream, errn, pre, fmt, ... )\
     sl_spew(level, stream, errn, pre, __BASE_FILE__, __LINE__,\
...
#  define DO_ASSERT(fname, val, ...) \
    do {\
        if(!((bool) (val))) {\
            ...
            sl_assertAction(SPEW_FILE, __BASE_FILE__, __LINE__, __func__);\
        }\
    }\
    while(0)
